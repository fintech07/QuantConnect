...
class CopulaPairsAlgorithm(QCAlgorithm):
    ...
    def Initialize(self):
        ...
        self.weight_v = 0.4       # desired holding weight of asset v in the portfolio
        self.coef = 0             # to be calculated: desired ratio of quantity_u / quantity_v
        self.window = {}          # stores historical price to calculate trading day's stock return
    ...
    def OnData(self, slice):
        ...
        # Placing orders: if v is relatively overpriced, sell v and buy u
        if MI_u_v < self.floor_CL and MI_v_u > self.cap_CL:
            
            # If held long position in v and short position in u, liquidate first and then place orders in opposite directions
            if self.Portfolio[long].Quantity < 0 and self.Portfolio[short].Quantity > 0:
                
                self.Liquidate()
                
                negQuantity = self.CalculateOrderQuantity(self.pair[1], -self.weight_v) # return negative quantity for defined holding weight
                max_u = self.Portfolio.TotalPortfolioValue / self.Portfolio[long].Price # max quantity of u can be traded given current buying power
                max_v = max_u / self.coef # the corresponding max quantity of v can be traded
                quantity_v = min(-negQuantity, max_v)
                self.Sell(short, quantity_v)
                self.Buy(long, self.coef * quantity_v)
                
            # Else if already held some short position in v and long position in u
            else:
                
                negQuantity = self.CalculateOrderQuantity(short, -self.weight_v) # return additional signed quantity needed
                
                # if v has already reached desired holding weight in short position, then return
                if negQuantity >= 0:
                    return
                
                # otherwise, sell more v and buy more u
                max_u = self.Portfolio.TotalPortfolioValue / self.Portfolio[long].Price # max quantity of u can be traded given current buying power
                max_v = max_u / self.coef # the corresponding max total quantity of v can be traded
                quantity_v = min(-negQuantity, max_v - abs(self.Portfolio[short].Quantity))
                self.Sell(short, quantity_v) 
                self.Buy(long, self.coef * quantity_v)
            
            self.Log(f"OnData on {self.Time} :: {long.Value}/{short.Value} : {self.coef}")
                
        # Placing orders: else if v is relatively underpriced: buy v and sell u
        elif MI_u_v > self.cap_CL and MI_v_u < self.floor_CL:
                
            # If held short position in v and long position in u, liquidate first and then place orders in opposite directions
            if self.Portfolio[long].Quantity > 0 and self.Portfolio[short].Quantity < 0:
                
                self.Liquidate()
                
                posQuantity = self.CalculateOrderQuantity(short, self.weight_v) # return positive quantity for defined holding weight
                max_u = self.Portfolio.TotalPortfolioValue / self.Portfolio[long].Price # max quantity of u can be traded given current buying power
                max_v = max_u / self.coef # the corresponding max quantity of v can be traded
                quantity_v = min(posQuantity, max_v)
                self.Buy(short, quantity_v) 
                self.Sell(long, self.coef * quantity_v)
                
            # Else if already held some long position in v and short position in u
            else:
                posQuantity = self.CalculateOrderQuantity(short, self.weight_v) # return additional signed quantity needed
                
                # if v has already reached desired holding weight in long position, then return
                if posQuantity <= 0:
                    return
                
                # otherwise, buy more v and sell more u
                max_u = self.Portfolio.TotalPortfolioValue / self.Portfolio[long].Price # max quantity of u can be traded given current buying power
                max_v = max_u / self.coef # the corresponding max quantity of v can be traded
                quantity_v = min(posQuantity, max_v - self.Portfolio[short].Quantity)          
                self.Buy(short, quantity_v) 
                self.Sell(long, self.coef * quantity_v)
    ...
    def SetSignal(self, slice):
        ...
        # Pull history log returns for the past self.numdays days
        ...
        self.Log(f"AIC for all copula families: {AIC}")
        self.Log(f"Selected copula family for current trading pair: {self.copula}")
        ...
        self.Log(f"SetSignal on {self.Time} :: {self.coef}")
    ...
    def PairSelection(self, date):
        ...
        self.Log(f"Selected pair: {[str(i) for i in self.pair]} with Kendall tau of {tau}.")
    ...
    def OnSecuritiesChanged(self, changes):
        ...
        for security in changes.RemovedSecurities:
            ...
            if self.Portfolio[symbol].Invested:
                self.Liquidate(symbol, "Removed Security")
